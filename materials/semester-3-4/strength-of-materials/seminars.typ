...
#grid(
  ...
  [
    ...
    $
      & sigma_max <= [sigma]; space [sigma] = sigma_"0,2"/[n] \
      ...
      & (48sqrt(M_y^2 + M_z^2 + M_"кр"^2))/(pi D^3) <= sigma_"0,2"/[n] => D >= root(3, (48[n] sqrt(M_y^2 + M_z^2 + M_"кр"^2))/(pi sigma_"0,2"))
    $
  ],
)
...
#grid(
  ...
  [
    ...
    $
    где $E$ -- модуль упругости, для изгиба -- модуль Юнга, для кручение -- модуль упругости второго рода (брать $G = "0,4"E$); $I$ -- момент инерции
