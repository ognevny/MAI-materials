...
#show: conf.with(
  title: "Семинары по Сопротивлению материалов (4 семестр)",
  description: "Семинары Феоктистовой Е.С. по Сопротивлению материалов",
...
#grid(
  ...
  [
    ...
    $display(integral_0^(pi/2) (M_I^1)^2 2l dif phi = integral_0^(pi/2) (1 dot 2l sin phi)^2 2l dif phi = \ = 8l^3 integral_0^(pi/2) sin^2 phi dif phi = 8l^3 pi/4 approx 6.28l^3)$

    Для треугольника $Omega h = 4/3 l dot 2l^2 = 8/3 l^3 approx 2.67l^3$

    $delta_11 = 8.95l^3/(E I)$

    $display(8.95l^3/(E I) dot X_1 - 4/3 (q l^4)/(E I) = 0 => X_1 approx 0.149q l)$
  ],
)
...
Нахождение моментов инерции#footnote[ВАЖНО: к сожалению здесь перепутаны местами оси $y$ и $z$. Вроде как если поменять $y$ на $z$, а $z$ на $y$ в индексах, то всё будет верно]
...
#grid(
  ...
  [
    #figure(
      image("source-figures/sem15-1.png"),
    )
  ],
  [
    Рассчитывается кольцевое сечение. На этот раз берётся вся рама. Берём самый нагруженный участок.

    $
      & sigma_max <= [sigma]; space [sigma] = sigma_0.2/[n] \
      & sigma^"III" = sqrt(sigma^2 + 4tau^2) \
      & sigma_"из" = +-M_z/W_z +- M_y/W_y \
      & W_z = W_y = W_"из" = omega delta = (pi D^2 delta)/4 \
      & tau = M_"кр"/W_"кр"; space W_"кр" = 2omega delta = 2W_"из" \
      & sigma^"III" = sqrt(sigma_z^2 + sigma_y^2 + 4tau^2) = sqrt(M_z^2/W_"из" + M_y^2/W_"из" + 4 M_"кр"/W_"из") = (48sqrt(M_y^2 + M_z^2 + M_"кр"^2))/(pi D^3) \
      & (48sqrt(M_y^2 + M_z^2 + M_"кр"^2))/(pi D^3) <= sigma_0.2/[n] => D >= root(3, (48[n] sqrt(M_y^2 + M_z^2 + M_"кр"^2))/(pi sigma_0.2))
    $
  ],
)
...
#grid(
  ...
  [
    #figure(
      image("source-figures/sem15-2.png"),
    )
  ],
  [
    Нахождение перемещения точки E: берём плоскость $C D E$. К точке $E$ перпендикулярно приложить единичную силу (направление не важно), построить эпюру моментов от этой силы.

    Считаем перемещение по интегралу Мора#footnote[Далее прям внимательно. В дальнейшем расчёте написано интегрирование по $dif l$, но $l$ здесь -- переменная длина от 0 до конца участка. В границах интегрирования же стоит $l$, которая является базовой длиной (на лекциях по урматам переменные меняли нормально...)]
    $
      delta = 1/(E I) sum_(i=1)^w integral_l_i M_i^1 M_i^P dif x,
    $
    где $E$ -- модуль упругости, для изгиба -- модуль Юнга, для кручение -- модуль упругости второго рода (брать $G = 0.4E$); $I$ -- момент инерции

    $display(
      delta = 1/(E I) integral_0^l M_(D E)^(1 x) dot M_(D E)^(P x) dif l + 1/(E I) integral_0^(2l) M_(C D)^(1 y) dot M_(C D)^(P y) dif l + 1/(G I_"кр") integral_0^(2l) M_(C D)^(1 "кр") times \ times M_(C D)^(P "кр") dif l
    )$

    $I_y$ берём из прямоугольного сечения ($84delta b^3$), $I_x$ -- из кольцевого ($(pi D^3)/8 delta$).

    $I_"кр" = (2omega^2 delta)/(b + h)$
  ],
)
...
#grid(
  ...
  [
    #figure(
      image("source-figures/sem16-1.png"),
    ) <гибкость>
  ],
  [
    При $0 < lambda < lambda'_"пр" quad sigma_к = sigma_т$ -- сжатие.

    При $lambda'_"пр" < lambda < lambda_"пр" quad sigma_"пц" <= sigma_к <= sigma_т$ меняется линейно -- пластичная деформация. Считается по формуле Ясинского.

    При $lambda > lambda_"пр"$ -- упругая деформация. Считается по формуле Эйлера.
  ],
)

Для пункта 2 нужно получить
$
  sigma <= [sigma] quad Delta sigma <= 5% quad Delta = (sigma - [sigma])/[sigma]
$

Для пункта 1 производим аппроксимацию функции $y$, формула энергетического метода
$
  P_к = (E I_min integral_l (y'')^2 dif x)/(integral_l_1 (y')^2 dif x)
$

При этом для поиска коэффициента приведенной длины $mu$ нужно воспользоваться формулой Эйлера
$
  P_к = (pi^2 E I_min)/(mu l)^2
$

$P_к$ и там, и там одинаково, поэтому

$
  (pi^2 E I_min)/(mu l)^2 = (E I_min integral_l (y'')^2 dif x)/(integral_l_1 (y')^2 dif x) => mu = pi/l sqrt((integral_l_1 (y')^2 dif x)/(integral_l (y'')^2 dif x))
$

Для подбора #underline[тригонометрической] функции используем уравнение кривизны
$
  dv(y, x, 2) = -M/(E I) => dv(y, x, 2) = -(F y)/(E I) => (dv(y, x))^2 + k y = 0 => y = С_1 cos k x + C_2 sin k x
$

#table(
  columns: 2,
  [], [Граничные условия],
  [Жёсткая заделка -- жёсткое закрепление], [$y(x) = 0$],
  [Неподвижный шарнир -- разрешает лишь вращательное движение], [$y(x) = y''(x) = 0$],
  [Защемление -- разрешает лишь вертикальное и горизонтальное перемещение], [$y'(x) = y'''(x) = 0$],
  [Втулка -- разрешает лишь вертикальное перемещение], [$y'(x) = y''(x) = 0$],
)

где $y(x)$ -- перемещение, $y'(x)$ -- угол поворота, $y''(x)$ -- момент, $y'''(x)$ -- перерезывающая сила.

В случае подбора #underline[полинома] берём такую функцию
$
  y = a_0 + a_1 x + a_2 x^2 + a_3 x^3 + a_4 x^4
$
Максимум 4 степень, поскольку у нас граничные условия вплоть до третьей производной.

В обоих случаях считаем соответствующие производные, подставляем граничные условия, находим соответствующие коэффициенты (обычно принимают в случае тригонометрии $C = a$, одна из "цэшек" должна занулиться).

_Рекомендация от преподавателя: сначала посчитать отношение интегралов, пусть оно равно $xi$, далее подставляем $ xi = pi^2/(mu^2 l^2), $ находим $mu$_

Для пункта 2 находим предельное значение гибкости
$
  lambda_"пр" = sqrt((pi^2 E)/sigma_"пц")
$
$lambda'_"пр" = 0.4lambda_"пр"$

Для каждого приближения считаем
$
  lambda = (mu l)/i_min,
$
где $i_min$ -- минимальный радиус инерции

При этом учитываем то, как соотносится $lambda$ с $lambda_"пр"$ и $lambda'_"пр"$ (см. @гибкость)

Для первого приближения считаем сжатие из
$
  sigma = P/F => F_"потр" = P/[sigma]
$
Учитываем, что нужно ещё делить на количество сечений, тогда мы получим потребную площадь для сортамента.

$
  i_min = sqrt(I/F),
$
где $I$ считается исходя из сечения, $F$ -- суммарная площадь всего сечения

Допустим мы попали в область Ясинского, тогда
$
  sigma_к = a + b lambda^((n)) \
  a = (sigma_т lambda_"пр" - sigma_"пц" lambda'_"пр")/(lambda_"пр" - lambda'_"пр") \
  b = (sigma_т - sigma_"пц")/(lambda_"пр" - lambda'_"пр") \
$

При этом в этой области $[n]_y$ считаем как
$
  [n]_y = A + B (lambda^((n))/lambda_"пр")^2,
$
где $A = 1.8$ и $B = 1.2$. Учитывать, что в области Эйлера $[n]_y = 3$.
