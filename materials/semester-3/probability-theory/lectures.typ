...
#show: conf.with(
  title: "Лекции по Теории вероятностей",
  description: "Конспект лекций Рудненко А.В. по Теории вероятностей",
  date: datetime(year: 2026, month: 1, day: 21),
...
$x_0.5 = M_e (X) \
x_0.1, x_0.2, ... space dash "децили" \
x_0.01, x_0.02, ... \
x_0.25, x_0.5, x_0.75 space dash "квартили" \
x_(1-q): P(X >= x_(1-q)) = q$
...
$display(
  ...
  x_0.3: F(x_0.3) = x_0.3^3 = 0.3 => x_0.3 = root(3, 0.3) approx 0.67 \
...
+ $x_0.7: F(x_0.7) = 1 - P(X >= x_0.7) = 1 - P(x_0.7 <= X < +oo) = 1 - (Phi((+oo - 173)/6) - Phi((x_0.7 - 173)/6)) = 0.7 <=> \ <=> Phi((x_0.7 - 173)/6) = 0.2 <=> Phi(t) = 0.2 <=> t = 0.525 <=> x_0.7 = 6t + 173 = 6 dot 0.525 + 173 approx 176$
...
$display(
  ...
  sigma(X_1) = 1.483E(X_1) \
  sigma(Y_1) = 1.483E(Y_1)
)$
...
$"а)" k = 1, P((X, Y) in D_э_1) = 1 - e^(-rho^2) approx 0.203 quad E_R = 1.75E \
"б)" k = 4, P((X, Y) in D_э_4) = 1 - e^(-16rho^2) approx 0.974$

$
  circle k = R/E, P((X, Y) in D_"кр") = 1 - e^(-rho^2), E = rho sqrt(2) sigma = 0.6744sigma
$
...
#grid(
  ...
  [
    $P(macron(X) <= a - 8) = P(-oo < macron(X) <= a - 8) = \ = P display(((-oo - a)/(sigma sqrt(n)) n < (macron(X) - a)/(sigma sqrt(n)) < ((a - 8) - a)/(sigma sqrt(n)) n)) = Phi(display(-8/sigma) sqrt(n)) - Phi(-oo) = \ = Phi(+oo) - Phi(display((8 sqrt(500))/40)) = 0.5 - 0.499997 approx 0.000003"г"$
  ],
)
...
#grid(
  ...
  [
    ...
    $n space dash space ? \
    X >= 10 \
    P_д = 0.98 = 0.5 - Phi((10 - 1.55n)/sqrt(1.65n)) \
    Phi((10 - 1.55n)/sqrt(1.65n)) = -0.48 \
    -(10 - 1.55n)/sqrt(1.65n) = 2.05 => n = 12.433 => 13$
  ],
)
