...
#import "meta.typ": conf, const, delimsys, ov

#show: conf.with(
  title: "Ответы на вопросы к экзамену по Теоретической механике",
  description: "Ответы на вопросы к экзамену по Теоретической механике у Зародова В.К.",
...
+ Сумма внутренних сил МС равна 0 (при доказательстве использовать III закон Ньютона)
+ Сумма моментов внутренних сил относительно любого центра равна 0 (при доказательстве посчитать моменты)
...
$display(
  dv(ov(K)_(j O), t) = dv(, t) sum_j [ov(r)_j, m_j ov(V)_j] = sum_j dv(, t) [ov(r)_j, m_j ov(V)_j] = sum_j ([dv(ov(r)_j, t), m_j ov(V)_j] + [ov(r)_j, dv((m_j ov(V)_j), t)]) = \ = sum_j [ov(r)_j, ov(F)_j^((e))] + sum_j [ov(r)_j, ov(F)_j^((i))] = sum_j ov(M)_0 (ov(F)_j^((e))) + cancel(sum_j ov(M)_0 (ov(F)_j^((i))))
) \
...
$display(dv(K_(O z), t) = sum_j M_(O z) (ov(F)_j^((e))))$
...
Поле сил называется потенциальным, если существует некая функция $U$, зависящая от координат ($exists U(x, y, z, ..., x_n, y_n, z_n)$), такая что выполняются уравнения $ F_(j x) = pdv(U, x_j), F_(j y) = pdv(U, y_j), F_(j z) = pdv(U, z_j) $ для $j = 1...n$. Соответственно $U$ -- силовая функция.
...
Главный момент: $ ov(M)_C^(Phi) = -dv(ov(K)'_C, t) "и" ov(M)_O^(Phi) = -dv(ov(K)_O, t) $
...
$ ov(V)_j = display(sum_(i=1)^n pdv(ov(r)_j, q_i) dot(q_i) + pdv(ov(r)_j, t)) $
...
Если в положении равновесия консервативной МС потенциальная энергия имеет изолированный минимум, то положение равновесия является устойчивым $(display(pdv(Pi, q, 2) > 0))$.
...
_Здесь и далее под интегрированием уравнения движения подразумевается решение ДУ_.
...
Уравнение Мещерского в случае одновременного отсоединения и присоединения частиц $ m(t) dv(ov(V), t) = ov(F)^((e)) - dv(m_1, t) ov(V)_(1r) + dv(m_2, t) ov(V)_(2r) $
...
Теорема об изменении кинетического момента: $ dv(K_z, t) = M_z^((e)) $
Кинетический момент относительно неподвижной оси: $ K_z = J_z omega $
Кинетический момент материальной точка: $ K_z = +- h m V, $ где h -- длина перпендикуляра, проведенного от оси к вектору скорости

Необходимо знать моменты инерции однородного диска и цилиндра ($J_z = 1/2 M R^2$); стержня, ось вращения в центре стержня ($J_z = 1/12 M L^2$); стержня, ось вращения в конце стержня ($J_z = 1/3 M L^2$)

$ M_z^((e)) = 0 => K_z = const $

Теорема об изменении кинетической энергии в интегральной форме: $ Delta T = Delta Pi + A_((1))^((2)*), $ где $A_((1))^((2)*)$ -- работа непотенциальных сил; $Delta Pi = M g Delta h$ в случае силы тяжести.

Кинетическая энергия при поступательном движении: $ T = (M V^2)/2 $
При вращательном: $ T = (J_z omega^2)/2 $
При плоскопараллельном (формула Кёнига): $ T = 1/2 M V_c^2 + 1/2 J_c omega^2 $

Принцип д'Аламбера для материальной точки: $ ov(Phi) = -m ov(W) $ (во многих задачах ускорение задано!)

Принцип виртуальных перемещений (использовать, когда система находится в равновесии): $ sum_j (ov(F)_j, delta ov(r)_j) +- M delta phi = 0 $
+ Тело вращается вокруг неподвижной точки: все перемещения перпендикулярны радиусу $ delta r_A = delta phi dot O A, $ где $O A$ -- расстояние от центра до точки $A$
+ Движение твердого тела: $ "Пр"_(ov(A B)) delta ov(r)_A = "Пр"_(ov(A B)) delta ov(r)_B "или" (delta r_A)/(A P) = (delta r_B)/(B P) = ... $
+ Перемещения двух точек твердого тела параллельны в данный момент времени $ delta ov(r)_A = delta ov(r)_B $

Уравнение Лагранжа при одной степени свободы (одна обобщённого координата). В задачах обычно требуется нахождение ускорения. В уравнение Лагранжа это ускорение должно войти.
