...
$ (1 - M^2) pdv(u, x, [2]) + pdv(u, y, [2]) + pdv(u, z, [2]) = 0, $ $u(x, y, z)$ -- потенциал скорости, $M$ -- число Маха, $display(evaluated(pdv(u, n))_r = 0)$.
...
$phi_1(x)$, $phi_2(x)$ -- первые интегралы системы ОДУ

$P(x, y, z) pdv(z, x) + Q(x, y, z) pdv(z, y) - R(x, y, z) = 0$

Векторы ${P, Q, R}$, $display(n = {pdv(z, x), pdv(z, y), -1})$ -- вектор нормали к поверхности
...
Ответ: $display(z = 1/2 (x + y)^2 + 1/(x^2 + y^2) - 1/2 (x^2 + y^2) = x y + 1/(x^2 + y^2))$
...
$u(x, t)$ -- отклонение струны от положения равновесия точки $x$
...
Найти решения $u_(t t) = a^2 u_(x x)$ в области $cases(delim: #none, -oo < x < oo, 0 <= t < oo)$ при заданных условиях $evaluated(u)_(t=0) = phi(x) quad evaluated(u_t)_(t=0) = psi(x)$ -- постановка задачи Коши.
