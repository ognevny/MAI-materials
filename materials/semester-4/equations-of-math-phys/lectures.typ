...
Пусть $xi = phi(x, y)$ -- решение УЧП (1), тогда $phi(x, y) = C$ является первым интегралом дифференциального уравнения
...
Замены подобраны таким образом, что
$
...
$display(
  ...
  (dv(y, x) + 1)^2 = 0 => dv(y, x) = -1 => dif y = -dif x => y = -x + C => C = x + y => cases(xi = y + x, eta = x) space cases(
    delim: "|",
    xi_x = 1 quad xi_y = 1,
...
Путём обратной замены получим общее решение $#rect[$ u(x, y) = tilde(psi)(y + x) e^(-x) + phi(y + x) $]$ , где $tilde(psi)$ и $phi$ -- произвольные функции

$display(
  "Рассмотрим альтернативный способ (работает при постоянных коэффициентах)" \
  u''_(eta eta) + u'_eta = 0 \
  y'' + y' = 0 \
  k^2 + k = 0 <=> k (k + 1) = 0 <=> k = 0\, space k = 1 \
  y = C_1(xi) + C_2(xi) e^(-eta) => u = C_1(y + x) + C_2(y + x) e^(-x)
)$

Также можно заменить первую производной другой функцией

$display(
  v = u_eta => v_eta + v = 0 <=> dv(v, eta) = -v <=> (dif v)/v = -dif eta <=> ln abs(v) = -eta + ln abs(phi(xi)) <=> v = tilde(phi)(xi) e^(-eta) <=> dv(u, eta) = tilde(phi)(xi) e^(-eta) <=> \ <=> dif u = tilde(phi)(xi) e^(-eta) dif eta <=> u = tilde(phi)(xi) (-e^(-eta)) + psi(xi) \
  u = Phi(xi) e^(-eta) + psi(xi)\, "где" Phi(xi) = -tilde(phi)(xi)\, space tilde(phi)(xi) = +- phi(xi)
)$
...
Найти общее решение уравнения $#rect[$ u_(x x) - 4u_(x y) - 21u_(y y) + 10u_x + 30u_y = 0 $]$
...
Ответ: $#rect[$ u(x, y) = e^(x + 7y) Phi(y - 3x) + psi(y + 7x) $]$
...
$display(
  ...
  -4a^2 u_(xi eta) = 0 <=> u_(xi eta) = 0 <=> dv(u_xi, eta) = 0 <=> u_xi = psi(xi) <=> dv(u, xi) = psi(xi) <=> integral dif u = integral(psi(xi)) dif xi <=> u = Psi(xi) + phi(eta) \
  #rect[$ u(x, t) = Psi(x + a t) + phi(x - a t) $]
)$
...
Подставим общее решение и найдём соотношение с функциями $phi$ и $psi$
...
Найти решение уравнения $#rect[$ u_(t t) = 4u_(x x) space cases(
  delim: bar.v.double,
...
Найти решение уравнения $#rect[$ u_(t t) = 4u_(x x) + sin t space cases(
  delim: bar.v.double,
...
+ $ov(lambda) = lambda^2 > 0$
  ...
  X(l) = 0 + C_2 sin(lambda l) = 0 stretch(=>)^(C_2 != 0) sin(lambda l) = 0 => lambda l = pi k => lambda = (pi k)/l => ov(lambda) = lambda^2 = ((pi n)/l)^2 space dash "собственные числа" => \ => X_n (x) = C_2 sin (pi n)/l x space dash "собственные функции"$

Задача по нахождению собственных функций $X(x)$ из условия $X'' + ov(lambda) X = 0$ называется задачей Штурма-Лиувилля.

$T'' + a^2 ov(lambda) T = 0 <=> T'' + a^2 lambda^2 T = 0 \
k^2 + a^2 lambda^2 = 0 <=> k_(1,2) = +-a lambda \
T = C_1 cos(a lambda t) + C_2 sin(a lambda t) => T_n (t) = tilde(A)_n cos (pi n)/l a t + tilde(B)_n sin (pi n)/l a t$
...
Отсюда $phi(x)$ и $psi(x)$ получаем разложенными в ряд Фурье по синусам
...
$display(
  ...
  "Ответ:" #rect[$ u(x, t) = sum_1^oo 2/(pi n) dot (-1)^n cos n t sin n x $] "при" #rect[$ A_n = 2/(pi n) dot (-1)^n $] и #rect[$ B_n = 0 $]
)$
...
$display(
  sum_1^oo T''_n (t) sin n x = sum_1^oo T_n (t) (-sin n x dot n^2) + f(x, t) <=> sum_1^oo [T''_n (t) sin n x + sin n x dot n^2] = f(x, t) <=> \ <=> sum_1^oo [T''_n (t) + n^2] sin n x = f(x, t) => sum_1^oo [T''_n (t) + n^2] sin n x = sin 3x
)$
...
=== Первая краевая задача для однородного уравнения теплопроводности
...
Общее решение $#rect[$ u(x, t) = sum_1^oo 4/(5pi (2k - 1)) e^(-(pi (2k -1))^2 t) sin (pi (2k - 1) x)/5 $]$ при $#rect[$ A_n = 4/(5pi (2k - 1)) $]$
...
+ $ov(lambda) > 0 => lambda^2 = ov(lambda) => X'' + lambda^2 X = 0$
  ...
  $display(
    ...
    lambda l = pi n => lambda = (pi n)/l => ov(lambda) = ((pi n)/l)^2 => X_n = C_1 cos (pi n x)/l \; quad X_0 = C_1
  )$
...
$
  u(x, t) = tilde(A)_0 C_1 + sum_1^oo tilde(A)_n C_1 e^(-a^2 ((pi n)/l)^2 t) cos (pi n x)/l = #rect[$ A_0/2 + sum_1^oo A_n e^(-a^2 ((pi n)/l)^2 t) cos (pi n x)/l $]
$
...
$display(
  ...
  "Ответ:" #rect[$ u(x, t) = pi/2 + sum_1^oo -4/(pi (2k - 1)^2) e^(-4 (2k - 1)^2 t) cos (2k - 1) x $] "при" #rect[$ A_0 = pi $] и #rect[$ A_n = -4/(pi (2k - 1)^2) $]
)$
...
$display(
  ...
  A_n = 2/pi integral_0^pi cos 2x cos (pi n x)/pi dif x = cases(
    0 quad & n != 2,
    1 quad & n = 2,
  ). "Функция" phi(x) "уже разложена в ряд Фурье"
)$

Ответ: $#rect[$ u(x, t) = e^(-4 dot 2^2 t) cos 2x = e^(-16t) cos 2x $]$ при $#rect[$ A_0 = 0 $]$ и $#rect[$ A_n = 1 $]$
...
$display(
  ...
  u'_t = sum_0^oo T'_n cos n x \; quad u''_(x x) = sum_0^oo T_n (-n^2 cos n x) \
  sum_0^oo T'_n cos n x = 1/9 sum_0^oo T_n (-n^2 cos n x) + cos 3x \
  sum_0^oo [T'_n + 1/9 n^2 T_n (t)] cos n x = sum_0^oo I_n cos n x \, space I_3 = 1
)$

$display(
  "Для" & cos n x: T'_n (t) + 1/9 n^2 T_n (t) = 0 quad T_n (0) = 0 \
  & cos 3x: T'_3 (t) + 1/9 dot 3^2 dot T_3 (t) = 1 quad T_n (0) = 0
)$
...
$display(
  ...
  "Для" (1) "имеем" ov(lambda) = lambda^2 >= 0 => Phi'' + lambda^2 Phi = 0 => Phi(phi) = tilde(A) cos lambda phi + tilde(B) sin lambda phi
)$
...
Функция $Phi(phi)$ -- периодическая с периодом $2pi$. Отсюда $lambda = n in NN$ (и ноль), то есть $ Phi_n (phi) = tilde(A)_n cos n phi + tilde(B)_n sin n phi $
...
$display(
  ...
  "Ответ:" #rect[$ u = 1/2 + sum_1^oo r^(2k - 1)/2^(2k - 2) dot (-1)^(k-1)/(pi (2k - 1)) cos (2k - 1) phi $] "при" #rect[$ A_n = (-1)^(k-1)/(pi dot 2^(2k-2)) $] "и" #rect[$ B_n = 0 $]
)$
...
Найти вариацию функционала $#rect[$ J[y(x)] = integral_0^pi y'^2 sin x dif x $]$
